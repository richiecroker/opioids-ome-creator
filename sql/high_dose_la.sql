...
WHERE
  (
    (
      ...
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 37.5)  -- Fentanyl patches (strengths equal to or higher than 37.5mcg/hour) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 387024006
      AND route.descr LIKE '%modified-release.oral'
      AND vmp_amp.nm NOT LIKE '%Onexila%40mg%'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 30)  -- Oxycodone MR oral preps [excluding Onexila 40mg as 24hr] (strengths equal to or higher than 30mg) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 441757005
      AND route.descr LIKE '%modified-release.oral'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 112.5)  -- Tapentadol base substance (strengths equal to or higher than 112.5mg) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 387173000
      AND route.descr LIKE 'patch.transdermal'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 39.375)  -- Buprenorphine patches (strengths equal to or higher than 39.375mg/hour) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 60886004
      AND route.descr LIKE '%modified-release.oral'
      AND vmp_amp.nm NOT LIKE '%MXL%'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 45)  -- Morphine Sulfate MR oral preps [excluding MXL as 24hr] (strengths equal to or higher than 45mg) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 60886004
      AND route.descr LIKE '%modified-release.oral'
      AND vmp_amp.nm LIKE '%MXL%'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 90)  -- MXL (strengths equal to or higher than 90mg) (ing code)
    OR (
      COALESCE(vpi.bs_subid, vpi.ing) = 44508008
      AND route.descr LIKE '%modified-release.oral'
      AND strnt_nmrtr_val / (COALESCE(strnt_dnmtr_val, 1))
        >= 9))  -- Hydromorphone base substance (strengths equal to or higher than 9mg) (ing code)
